...
@section('content')
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-serif font-bold text-slate-900">Mes Leads</h2>
            <p class="text-sm text-slate-500 mt-1">{{ $leads->total() }} prospects au total</p>
        </div>
    ...
    <!-- Filters -->
    <div class="mb-8 bg-white p-5 rounded-xl shadow-sm border border-slate-100">
        <form method="GET" class="flex flex-wrap lg:flex-nowrap items-end gap-5">
            ...
            <div class="w-1/2 lg:w-56">
                ...
                <select name="funnel_id" id="funnel_id" class="block w-full rounded-lg border-gray-200 bg-slate-50 text-sm focus:border-amber-500 focus:ring-amber-500 py-2.5">
                    <option value="">Tous mes tunnels</option>
            ...
            <div class="flex items-center gap-2">
                <button type="submit" class="inline-flex items-center rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 transition-colors">
                    Filtrer
                </button>
                @if(request()->hasAny(['status', 'funnel_id', 'search']))
                    <a href="{{ route('commercial.leads') }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                        Rezet
                    </a>
                @endif
            </div>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($lead->score >= 60)
                                        <span class="inline-flex items-center rounded-full bg-rose-50 px-2.5 py-1 text-xs font-medium text-rose-700 ring-1 ring-inset ring-rose-600/10">
                                            🔥 {{ $lead->score }} pts
                                        </span>
                                    @elseif($lead->score >= 30)
                                        <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-600/10">
                                            ☀️ {{ $lead->score }} pts
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-3">
                                        @if($lead->phone)
                                            ...
                                            target="_blank"
                                            class="group flex items-center justify-center h-8 w-8 rounded-full bg-green-50 text-green-600 hover:bg-green-100 hover:text-green-700 transition-all focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                                            title="WhatsApp">
                                        ...
                                        @if($lead->email)
                                            ...
                                            class="group flex items-center justify-center h-8 w-8 rounded-full bg-slate-100 text-slate-600 hover:bg-amber-100 hover:text-amber-700 transition-all focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2"
                                            title="Email">
