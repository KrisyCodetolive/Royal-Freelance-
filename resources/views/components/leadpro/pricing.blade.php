...
<section id="tarifs" class="py-24 bg-slate-50 relative overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="text-center mb-16">
            <div
                class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 text-amber-700 text-xs font-bold tracking-wider uppercase mb-4">
                Tarifs
            </div>
            <h2 class="text-3xl md:text-5xl font-serif font-bold text-slate-900 mb-4">Un plan pour chaque étape de
                votre croissance</h2>
            <p class="text-slate-600 max-w-2xl mx-auto text-lg">Commencez gratuitement, évoluez quand vous en avez
        ...
        <div class="grid md:grid-cols-3 gap-8 items-start">
            @foreach ($plans as $plan)
                ...
                @endphp
                <div
                    class="relative bg-white rounded-2xl p-8 border-2 {{ $recommended ? 'border-amber-400 shadow-2xl shadow-amber-500/10 md:-translate-y-4' : 'border-slate-100 shadow-sm' }} transition-all duration-300">
                    @if ($recommended)
                        <div
                    ...
                    <div class="mb-6">
                        ...
                        @else
                            <span class="text-4xl font-bold text-slate-900">{{ number_format($plan->price_monthly, 0, ',', ' ') }}</span>
                            <span class="text-slate-500"> FCFA/mois</span>
                            <div class="text-sm text-slate-400 mt-1">ou {{ number_format($plan->price_yearly, 0, ',', ' ') }} FCFA/an</div>
                        @endif
                    </div>
                    ...
                    <ul class="space-y-3 mb-8">
                        ...
                        <li class="flex items-start gap-2 text-sm text-slate-600">
                            <svg class="w-5 h-5 {{ $plan->max_shared_tunnels === 0 ? 'text-slate-300' : 'text-emerald-500' }} flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                            </svg>
                            <span class="{{ $plan->max_shared_tunnels === 0 ? 'text-slate-400' : '' }}">
                    ...
                    <a href="{{ route('tenant.register') }}"
                        class="block text-center px-6 py-3.5 rounded-lg font-bold transition-all duration-300 {{ $recommended ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-lg shadow-amber-600/20 hover:-translate-y-0.5' : 'bg-slate-100 hover:bg-slate-200 text-slate-900' }}">
