@php
    ...
    // Prioriser le WhatsApp du commercial si disponible
    ...
    $whatsappPhone = preg_replace('/[^0-9]/', '', $whatsappNumber);
    ...
    $url = match ($actionType) {
        ...
        'whatsapp' => 'https://wa.me/' . $whatsappPhone . '?text=' . urlencode($whatsappMessage),
        'whatsapp_group' => $content['whatsapp_group_url'] ?? '#',
        'shop' => $shopUrl,
        default => $content['url'] ?? '#',
    };

    // Force le numéro du commercial même si l'utilisateur a collé un lien wa.me en URL externe
    if ($actionType === 'url' && !empty($url) && str_contains($url, 'wa.me') && !empty($whatsappPhone)) {
        $url = 'https://wa.me/' . $whatsappPhone . '?text=' . urlencode($whatsappMessage);
    }
...
<div style="{{ $containerStyle }}">
    <a href="{{ $url }}" target="{{ $target }}" class="cta-button {{ $fullWidth ? 'w-full' : '' }}"
        ...
        data-button-id="{{ $block->id ?? 'cta' }}"
        onmouseover="this.style.opacity='0.9'; this.style.transform='translateY(-2px)'"
        onmouseout="this.style.opacity='1'; this.style.transform='translateY(0)'"
        onclick="trackCtaButtonClick(this)">
...
<script>
function trackCtaButtonClick(el) {
    var leadId = {{ $currentLead->id ?? 'null' }};
    var pageId = el.getAttribute('data-page-id');
    var buttonId = el.getAttribute('data-button-id');

    if (!leadId || !pageId) return;

    // Fire and forget — non-blocking
    fetch('{{ url("/api/tracking/cta-click") }}', {
        method: 'POST',
        headers: {
