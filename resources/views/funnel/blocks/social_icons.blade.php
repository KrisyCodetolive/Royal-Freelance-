...
@php
    $links = $content['links'] ?? [];
    $size = $content['size'] ?? 'medium'; // small, medium, large
    $style = $content['style'] ?? 'filled'; // filled, outline, minimal
    ...
    <div class="flex justify-center gap-4 flex-wrap">
        @foreach($links as $link)
            @php
                ...
                $url = $link['url'] ?? '#';
                $bgColor = $platformColors[$platform] ?? '#6B7280';

                // Attribution commerciale pour WhatsApp
                if ($platform === 'whatsapp' && !empty($commercialWhatsApp)) {
                    $cleanPhone = preg_replace('/[^0-9]/', '', $commercialWhatsApp);
                    $msg = $commercialWhatsAppMessage ?? ($funnel->whatsapp_message ?? 'Bonjour!');
                    $url = "https://wa.me/{$cleanPhone}?text=" . urlencode($msg);
                }
            @endphp
            <a href="{{ $url }}" 
               target="_blank" 
               rel="noopener"
               class="{{ $iconSize }} flex items-center justify-center rounded-full transition-transform hover:scale-110
                      {{ $style === 'filled' ? '' : ($style === 'outline' ? 'border-2' : '') }}"
               style="{{ $style === 'filled' ? 'background-color: ' . $bgColor : ($style === 'outline' ? 'border-color: ' . $bgColor . '; color: ' . $bgColor : 'color: ' . $bgColor) }};">
